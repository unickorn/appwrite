...
trait StorageBase
{
    ...
    public function testCreateBucketFileXzCompression(): array
    {
        $bucket = $this->client->call(Client::METHOD_POST, '/storage/buckets', [
            'content-type' => 'application/json',
            'x-appwrite-project' => $this->getProject()['$id'],
        ...
        $this->assertEquals(201, $bucket['headers']['status-code']);
        $this->assertNotEmpty($bucket['body']['$id']);
        $this->assertEquals('xz', $bucket['body']['compression']);
    ...
    /**
     * @depends testCreateBucketFileXzCompression
     */
    public function testFilePreviewXzCompression(array $data): array
    {
        $bucketId = $data['bucketId'];
